...
> Theme ชื่อ **"เช้าวันใหม่"** (UX-01) — ครีมอุ่น + มัทฉะ + แอปริคอต · เดิมยืมพาเลตต์จากเว็บการตลาดของ Cohere ซึ่งเย็นและเป็นทางการเกินกว่าเสียงของแอป
...
- **นอน = `--chart-1` ฟ้าคราม · กิน = `--chart-2` อำพัน · เคลื่อนไหว = `--chart-3` เขียวสด** — สีถูกเลือกให้ตรงกับความหมาย อ่านกราฟออกโดยไม่ต้องดู legend
...
3. **สีหลัก = มัทฉะ `#1d6b53`** (`bg-primary`) — ปุ่มหลัก/จุดเน้น · พื้นหลังครีม `#fdfaf5` การ์ดขาว · ไฮไลต์ = แอปริคอต `#ffd9b8` (`bg-accent`)
4. **สีกราฟ**: `--chart-1` ฟ้าคราม (นอน) · `--chart-2` อำพัน (กิน) · `--chart-3` เขียวสด (ขยับ) · `--chart-4` ลาเวนเดอร์ · `--chart-5` ปะการัง
...
- อย่าทำหน้าแอปเป็น landing page — ยึด: พื้นครีม การ์ดขาวเรียบ ไม่มีเงาหนัก ช่องว่างเยอะ ปุ่ม pill มัทฉะ
